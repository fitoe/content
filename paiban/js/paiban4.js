...
//检测是否有重名对象
function Checkname(){
	$("#picbox .pic").not(".selected").each(function(index, element) {
...
function() {
	$(".pic").hide();
	$("#picbox .pic").each(function(index, element) {
...
//点击图片/重命名的时候
$(".pic").livequery( 'click', function(event) {
	selpic(this);
	});
	
//双击显示大图
$(".pic").livequery( 'dblclick', function() {
	$("#dialog").show();
	Dialoger(this);
...
$("#dialog .dia_title").livequery( 'change', function () {
	$(".pic .photo img[bigsrc='"+$("#dialog .dia_img").attr("src")+"']").next().text($(this).val());
	savepic();
	RefreshRenameProgress();
	 });
//详情页规格编辑后应用到项目
...
$("#dialog	li").livequery( 'click', function () {
	$("#dialog	li").removeClass("checked");
	 ...
	 var obj=$(".pic .photo img[bigsrc='"+$("#dialog .dia_img").attr("src")+"']").parentsUntil(".pic").parent();
...
key('ctrl+a', function(e){
	e.preventDefault();
	selpic($(".pic[fenlei='"+$('#foldersbox .selected').attr('name')+"']"));
	});
//删除
key('del', function(){
...
function ResizeFoldersbox(){
	var x=($("#foldersbox .folder").size()+2)*114-80
	$("#foldersbox").width(x);
	$("#foldermain").mCustomScrollbar({
		 ...
		 scrollAmount:1,
		 advanced:{ updateOnSelectorChange: true,autoExpandHorizontalScroll:true, updateOnContentResize: true },
		 scrollInertia:0,
