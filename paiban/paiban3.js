...
function() {
	...
	$("#showdown").text("");
	$("#showdown").append("<a href=/download.php?action=downtxtrename&catid="+thecatid+"&id="+theid+"><b>《重命名文件》</b> </a>");
	$("#showdown").append(" 分类排版文件：")
	$("#foldersbox .text").each(function(){
		$("#showdown").append("<a href=/download.php?action=downfenlei&catid="+thecatid+"&id="+theid+"&item="+$(this).text()+">"+$(this).text()+"</a> | ")
		});
...
//双击显示大图
$(".pic .photo").livequery( 'dblclick', function() {
	$("#dialog").dialog({
		//appendTo:"#dialog",
	...
	$("#dialog").find("img").attr("src",$(this).find("img").attr("src"));
	$("#dialog").show();
});
